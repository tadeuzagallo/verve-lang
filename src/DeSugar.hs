...
desugar_interface (Interface name var fns) =
  ExprDecl <$> (FunctionExpr <$> concatMap (desugar_interface_fn name var) fns)

desugar_interface_fn :: TcId -> TcId -> InterfaceFunction TcId -> [Function TcId]
desugar_interface_fn (TcId name _) _ (ConcreteFunction fn@(Function { fn_name=(Loc _ (TcId fname _)) })) = 
  return fn { fn_name=loc_id (name ++ "$" ++ fname) }

desugar_interface_fn (TcId iname _) _ (AbstractFunction (Prototype (TcId name t) (FnType vars params ret_type))) =
  [Function
    (loc_id $ iname ++ "$" ++ name)
    Nothing
    ((foldl toParam [] params) ++ [FunctionParameter (loc_id "$$dict$$") (length params) Nothing])
    Nothing
    (Block [
    Call
      (Call (var "at") (loc [var iname, arg (length params)]))
